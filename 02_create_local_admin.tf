...
resource "azuread_user" "local_admin" {
  ...
  password            = random_password.local_admin.result

  provider = azuread.target

}

resource "azuread_invitation" "local_admin" {
  ...
  message {
    ...
    password      :   ${random_password.local_admin.result}
    global admin  :   ${local.create_local_admin}
